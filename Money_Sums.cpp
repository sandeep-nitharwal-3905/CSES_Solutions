...
void solve(int idx, int sum, vector<int> &v, set<int> &st)
{
    if (idx == v.size())
    {
        if (sum)
    ...
    // take
    solve(idx + 1, sum + v[idx], v, st);
    // not_take
    solve(idx + 1, sum, v, st);
}
...
int32_t main()
{
    ...
    while (tt--)
    {
        ...
        sort(v.begin(),v.end());
        int mxsum = 100 * 1000;
        vector<vector<int>> dp(n + 1, vector<int>(mxsum + 1, 0));
        for (int i = 0; i < n; i++)
        {
            dp[i][v[i]] = 1;
        }
