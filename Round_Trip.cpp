...
pair<int, int> bfs(int s, vector<vector<int>> &g, vector<int> &vis, vector<int> &par)
{
...
int32_t main()
{
    ...
    while (tt--)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            if (vis[i] == -1)
            {
                pair<int, int> cycnode = bfs(i, g, vis, par);
                if (cycnode != make_pair(-1LL, -1LL))
                {
                    cyc = cycnode;
                    break;
                }
        ...
        // debug(cyc);
        if (cyc != make_pair(-1LL, -1LL))
        {
            int node1 = cyc.first;
            int node2 = cyc.second;
            if (vis[node1] > vis[node2])
            {
                swap(node1, node2);
            }
            int sz = vis[node2] - vis[node1];
            // debug(node1, node2, sz);
