...
int solve(int idx, int prev, int m, vector<int> &v, vector<vector<int>> &dp)
{
    if (idx == v.size())
    {
        return 1;
    ...
    if (v[idx] == 0)
    {
        vector<int> nval = {prev - 1, prev, prev + 1};
        for (int k = 0; k < 3; k++)
        {
            if (nval[k] > 0 && nval[k] <= m)
            {
                res += solve(idx + 1, nval[k], m, v, dp);
            }
            res %= M;
        }
    }
