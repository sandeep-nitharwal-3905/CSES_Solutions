...
string solve(int i, int j, vector<string> &v)
{
    if (i == 0 && j == 0)
    {
        string s = "";
    ...
    return min(up, left) + v[i][j];
}

string fun(int n, vector<string> &v, vector<vector<int>> &vis)
{
    ...
    while (!q.empty())
    {
        ...
        for (int i = 0; i < sz; i++)
        {
            ...
            if (curr.first + 1 < n)
            {
                mn = min(mn, v[curr.first + 1][curr.second]);
            }
            if (curr.second + 1 < n)
            {
                mn = min(mn, v[curr.first][curr.second + 1]);
            }
        }
        ans += mn;
        for (int i = 0; i < sz; i++)
        {
            ...
            if (curr.first + 1 < n && v[curr.first + 1][curr.second] == mn && !vis[curr.first + 1][curr.second])
            {
                vis[curr.first + 1][curr.second] = 1;
                q.push({curr.first + 1, curr.second});
            }
            if (curr.second + 1 < n && v[curr.first][curr.second + 1] == mn && !vis[curr.first][curr.second + 1])
            {
