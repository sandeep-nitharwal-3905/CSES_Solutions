...
int32_t main()
{
    ...
    while (tt--)
    {
        ...
        vector<int> room(n);
        int cnt = 1;
        st.insert({v[0].first.second, cnt});
        room[v[0].second] = cnt;
        for (int i = 1; i < n; i++)
        {
            auto it = st.lower_bound({v[i].first.first, -1});
            if (it == st.begin())
            {
                st.insert({v[i].first.second, ++cnt});
                room[v[i].second] = cnt;
            }
            else
            {
                ...
                st.erase(it);
                st.insert({v[i].first.second, roomno});
                room[v[i].second] = roomno;
            }
        }
        cout<<cnt<<endl;
        cout<<room;
