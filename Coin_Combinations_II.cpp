...
int solve(int idx, int m, vector<int> &v, vector<vector<int>> &dp)
{
    if (m == 0)
    {
        return 1;
    ...
    if (m >= v[idx])
    {
        take = solve(idx, m - v[idx], v, dp);
    }
    int not_take = solve(idx + 1, m, v, dp);
    return dp[idx][m] = (take + not_take) % M;
}
