...
int32_t main()
{
    ...
    while (tt--)
    {
        ...
        // debug(v);
        map<int, int> mp;
        mp[v[0].first.first] = v[0].second;
        for (int i = 1; i < n; i++)
        {
            auto it = mp.lower_bound(v[i].first.second);
            if (it == mp.begin())
            {
                mp[v[i].first.first] = max(v[i].second, mp[v[i - 1].first.first]);
            }
            else
            {
                it--;
                mp[v[i].first.first] = max(it->second + v[i].second, mp[v[i - 1].first.first]);
            }
            // debug(mp);
        }
        cout << mp[v[n - 1].first.first] << endl;
    }
    return 0;
}
