...
int32_t main()
{
    ...
    while (tt--)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            for (int j = 0; j < n; j++)
            {
                presum[i + 1][j + 1] += presum[i][j + 1] + (v[i][j] == '*');
            }
        }
        // debug(presum);
        for (int i = 0; i < n; i++)
        ...
        for (int i = 0; i < m; i++)
        {
            int a, b, c, d;
            cin >> a >> b >> c >> d;
            // debug(presum[c][d], presum[c][b - 1], presum[a - 1][d], presum[a - 1][b - 1]);
            cout << presum[c][d] - presum[c][b - 1] - presum[a - 1][d] + presum[a - 1][b - 1] << endl;
        }
        // debug(presum);
    }
    return 0;
