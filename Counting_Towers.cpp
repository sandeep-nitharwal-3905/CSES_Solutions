...
int solve(int lev, int flag, int n, vector<vector<int>> &dp)
{
    if (lev == n)
    {
        return 1;
    ...
    if (flag)
    {
        return dp[lev][flag] = (2 * solve(lev + 1, 1, n, dp) + solve(lev + 1, 0, n, dp)) % M;
    }
    else
    {
        return dp[lev][flag] = (4 * solve(lev + 1, 0, n, dp) + solve(lev + 1, 1, n, dp)) % M;
    }
}
...
int32_t main()
{
    ...
    while (tt--)
    {
        ...
        {
            int curr[2];
            curr[1] = (2LL * next[1] + next[0]) % M;
            curr[0] = (4LL * next[0] + next[1]) % M;
            next[0] = curr[0];
            next[1] = curr[1];
        }
        cout << (next[1] + next[0]) % M << endl;
    }
    return 0;
}
