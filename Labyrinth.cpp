    ...
    while (tt--)
    {
        ...
        while (!q.empty())
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                int nx = x + dx[i];
                int ny = y + dy[i];
                if (nx >= 0 && ny >= 0 && nx < n && ny < m && vis[nx][ny] == -1 && v[nx][ny] != '#')
                {
                    q.push({{nx, ny}, dis + 1});
                    vis[nx][ny] = dis + 1;
                    dir[nx][ny] = dch[i];
        ...
        cout << "YES" << endl;
        cout << vis[end.first][end.second] << endl;
        string ans = "";
        pair<int, int> curridx = end;
        while (curridx != start)
        {
            // debug(curridx, dir[curridx.first][curridx.second]);
            ans += dir[curridx.first][curridx.second];
            if (dir[curridx.first][curridx.second] == 'L')
            {
                curridx.second++;
            }
            else if (dir[curridx.first][curridx.second] == 'R')
            {
                curridx.second--;
            }
            else if (dir[curridx.first][curridx.second] == 'D')
            {
                curridx.first--;
            }
            else
