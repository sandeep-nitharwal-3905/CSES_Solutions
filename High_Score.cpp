...
int32_t main()
{
    ...
    while (tt--)
    {
        ...
        for (int i = 1; i < n; i++)
        {
            for (int j = 0; j < m; j++)
            {
                int u = eg[j].first.first;
                int v = eg[j].first.second;
                int w = eg[j].second;
                if (dis[u] != 1e18 && dis[u] + w < dis[v])
                {
                    dis[v] = dis[u] + w;
                }
                if (rdis[v] != 1e18 && rdis[v] + w < rdis[u])
                {
                    rdis[u] = rdis[v] + w;
                }
            }
        ...
        // debug(dis);
        // debug(rdis);
        int mxdis = dis[n - 1], negcyc = 0;
        for (int j = 0; j < m; j++)
        {
            int u = eg[j].first.first;
            int v = eg[j].first.second;
            int w = eg[j].second;
            if (dis[u] != 1e18 && dis[u] + w < dis[v] && rdis[v] != 1e18)
            {
                negcyc = 1;
            }
        }
...
iterate through all eges, how may times ? n-1 times.
if it is still reduce then negative cycle.
*/
