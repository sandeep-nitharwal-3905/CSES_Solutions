...
int solve(int idx, int sum, int total, int n, vector<int> &v)
{
    // base case
    if (idx == n)
    {
    ...
    // recursive
    // take&not_take
    return min(solve(idx + 1, sum + v[idx], total, n, v), solve(idx + 1, sum, total, n, v));
}
