...
int fun1(int i, int j, string &ans)
{
    if (i)
    {
        if ((ans[i - 1] != ('A' + j)))
...
int fun2(int i, int j, int n, vector<int> &v, set<pair<int, int>> &st, string &ans)
{
    int currf = v[j];
    st.erase({currf, j});
    st.insert({currf - 1, j});
    int mxf = (--st.end())->first;
    int mxe = (--st.end())->second;
    int len = n - i - 1;
    // debug(currf, j, mxf, len);
    if (j == mxe)
    {
        if (mxf <= ((len) / 2))
        {
    ...
    while (tt--)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            for (int j = 0; j < 26; j++)
            {
                if (v[j])
                {
                    if (fun1(i, j, ans) && fun2(i, j, n, v, st, ans))
                    {
                        ans[i] = ('A' + j);
                        break;
                    }
        ...
        int cnt = count(ans.begin(), ans.end(), 'x');
        if (cnt)
        {
            cout << -1 << endl;
        }
